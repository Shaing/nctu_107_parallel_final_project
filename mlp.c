...
void create_2_spirals_data_set_0(_2S* data, float* X, float* Y)
{
	for (int i = 0; i < 97; ++i)
	{
		data[i].X = X[i];
...
void create_2_spirals_data_set_1(_2S* data, float* X, float* Y)
{
	for (int i = 0; i < 97; ++i)
	{
		data[i].X = X[i];
...
/* test pattern */
	...
	create_2_spirals_data_set_0(c, X1_2s, Y1_2s);
	create_2_spirals_data_set_1(&c[97], X2_2s, Y2_2s);
...
/* initialize arrays */
	...
	nn_para.input_size = sizeof(c) / sizeof(_2S);
...
void init_input_weight(float* _w, int fl, int sl) // fl: first layer, sl: second layer
{
	...
	for (int x = 0; x < fl; ++x)
	{
		for (int y = 0; y < sl; ++y)
		{
			...
			_w[(x * y) + y] = distribution(generator);
